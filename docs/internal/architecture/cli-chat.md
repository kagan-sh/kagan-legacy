...
2. Add a handler branch in `ChatController._handle_slash_command()` in `controller.py`
3. Add a unit test in `tests/unit/test_chat_commands.py`

## Adding a new ChatEvent variant

1. Add the variant class to `src/kagan/core/chat/events.py`
2. Add it to the `ChatEvent` union type
3. Add a render branch in `StreamRenderer._render_event()` in `_streaming.py`
4. Add a unit test in `tests/unit/test_event_rendering.py`
