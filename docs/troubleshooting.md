...
| `claude-code initialization failed during handshake. Connection closed` (transient) | Usually resolved automatically: the long-lived ACP factory restarts the agent process once on broken-pipe / connection-reset / `RequestError`. If it recurs every turn, run `kg doctor`, then check `kagan.log` for the agent's stderr output and reinstall the backend if its binary is corrupt. |
