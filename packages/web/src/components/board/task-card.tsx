    ...
    <>
      <div className="ml-2 flex h-full min-h-0 flex-col gap-0.5">
        ...
        <div className="mt-auto flex items-center justify-between gap-1 bg-[color:var(--surface-1)] px-2 py-0.5 text-[9px] text-muted-foreground">
          <span className="inline-flex min-w-0 items-center truncate">{formatLastActivity(task.last_event_at || task.updated_at)}</span>
          {task.active_session ? (
...
export function TaskCardOverlayPreview({ task, className }: TaskCardOverlayPreviewProps) {
  return (
    <div
      className={cn(
  ...
  const cardContent = (
    <div
      ...
      className={cn(
        ...
        isSelected && 'ring-1 ring-[var(--primary)]/50 border-[var(--primary)]/30 bg-[color:var(--surface-2)]',
  ...
  return (
    <DropdownMenu open={contextMenuOpen} onOpenChange={setContextMenuOpen}>
      ...
      <DropdownMenuContent align="start">
        ...
        <DropdownMenuSeparator />
        <DropdownMenuItem variant="destructive" onSelect={() => onDeleteTask?.(task)}>
          <Trash2 />
          Delete
        </DropdownMenuItem>
      </DropdownMenuContent>
