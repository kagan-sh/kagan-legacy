...
function AppLayout() {
  ...
  const [pluginImportOpen, setPluginImportOpen] = useAtom(pluginImportOpenAtom);
  ...
  const setChatRailLayout = (mode: 'chat-right' | 'chat-bottom' | 'chat-fullscreen') => {
  ...
  const toggleAIPanel = useCallback(async () => {
    ...
    if (railOpen && (railMode === 'chat-right' || railMode === 'chat-bottom')) {
      ...
      try {
        ...
        const sid = orch.length > 0 ? orch[0]!.id : (await apiClient.createChatSession({})).id;
        setRailTaskId(null);
  ...
  }, [closeChatRail, currentTaskId, openChatRail, railChatSessionId, railMode, railTaskId, setChatRailLayout, setRailChatSessionId, setRailMode, setRailTaskId, setSessionPickerOpen]);
  ...
  }, [projectChecked, isMobile, location.pathname, railMode, setRailChatSessionId, setRailMode, setRailTaskId]);
  ...
  }, [projectVersion, isMobile, railMode, setRailChatSessionId, setRailMode, setRailTaskId]);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if ((event.metaKey || event.ctrlKey) && event.shiftKey && lowerKey === 'p') {
      ...
      if ((event.metaKey || event.ctrlKey) && event.shiftKey && lowerKey === 'k') {
        event.preventDefault();
      ...
      if (!isEditableTarget(event.target) && (event.key === '?' || event.key === 'F1')) {
        event.preventDefault();
        setCommandOpen(false);
        setSessionPickerOpen(false);
  ...
  return (
    <>
        ...
        <div className="relative flex min-w-0 flex-1 flex-col">
          {!isMobile && (
            <HeaderBar
              ...
              aiPanelOpen={railMode !== 'none' && Boolean(railTaskId || railChatSessionId)}
          ...
          <div className="flex min-h-0 min-w-0 flex-1 flex-col">
            <div className="flex min-h-0 min-w-0 flex-1">
              <main
                ...
                className={cn(
                  ...
                  railMode === 'chat-fullscreen' && 'overflow-hidden',
                )}
              >
                <Outlet />
              </main>

              {!isMobile && railMode === 'chat-right' && (railTaskId || railChatSessionId) ? (
            ...
            {!isMobile && railMode === 'chat-bottom' && (railTaskId || railChatSessionId) ? (
      ...
      {!isMobile && railMode === 'chat-fullscreen' && (railTaskId || railChatSessionId) ? (
