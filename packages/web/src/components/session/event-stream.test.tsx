...
describe('EventStream', () => {
  ...
  it('renders agent messages and tool calls', () => {
    const events = [
      mockEvent({ type: 'OUTPUT_CHUNK', payload: { text: 'Hello from the agent' } }),
      mockEvent({ type: 'TOOL_CALL_START', payload: { acp: { toolCallId: 'tc-1', toolName: 'read_file' } } }),
      mockEvent({ type: 'TASK_STATUS_CHANGED', payload: { from: 'BACKLOG', to: 'IN_PROGRESS' } }),
    ];
  ...
  it('renders user follow-up messages interleaved with events', () => {
    const events = [
      mockEvent({ type: 'OUTPUT_CHUNK', payload: { text: 'Working on it...' }, created_at: '2026-03-14T10:00:00Z' }),
    ];
    const followUps = [
  ...
  it('formats MCP tool names nicely', () => {
    const events = [
      mockEvent({ type: 'TOOL_CALL_START', payload: { acp: { toolCallId: 'tc-2', toolName: 'mcp__kagan__run_wait' } } }),
    ];
    renderWithProviders(<EventStream events={events} />);
  ...
  it('extracts tool name from alternate payload fields', () => {
    const events = [
      mockEvent({ type: 'TOOL_CALL_START', payload: { name: 'bash_tool', id: 'tc-3' } }),
    ];
    renderWithProviders(<EventStream events={events} />);
  ...
  it('does not show LIVE indicator when isRunning is false', () => {
    const events = [
      mockEvent({ type: 'OUTPUT_CHUNK', payload: { text: 'Done.' } }),
    ];
