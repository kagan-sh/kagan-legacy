...
export function useBoardDnd({ tasks, grouped, fetchTasks }: UseBoardDndOptions): UseBoardDndReturn {
  const [activeTask, setActiveTask] = useState<WireTask | null>(null);
  const [dragState, setDragState] = useState<Record<TaskStatus, WireTask[]> | null>(null);
  ...
  const handleDragOver = useCallback(
    (e: DragOverEvent) => {
      ...
      const fromStatus = COLUMN_ORDER.find((status) => source[status].some((item) => item.id === taskId));
  ...
  const handleDragEnd = useCallback(
    async (e: DragEndEvent) => {
      ...
      if (!COLUMN_ORDER.includes(targetStatus as TaskStatus)) return;
      if (!isAllowedTaskTransition(task.status as TaskStatus, targetStatus as TaskStatus)) {
        toast.error(
      ...
      try {
        await apiClient.transitionTaskStatus(taskId, targetStatus as TaskStatus);
        fetchTasks();
        toast.success(`Moved to ${STATUS_LABELS[targetStatus as TaskStatus]}`);
      } catch (error) {
