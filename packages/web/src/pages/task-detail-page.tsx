...
export function defaultTabForTask(task: WireTask): WorkspaceTab {
  ...
  if (task.status === 'DONE' && (task.review_verdicts?.length ?? 0) > 0) return 'review';
...
export function Component() {
  ...
  const showReviewTab = displayTask.status === 'REVIEW' || (displayTask.status === 'DONE' && hasVerdicts);
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1680px] flex-col gap-5 px-4 py-4 sm:px-6">
      ...
      <div className="flex items-center gap-2 border-b border-[color:var(--border-subtle)] pb-3">
        <Button variant="ghost" size="icon-sm" onClick={() => navigate('/board')} aria-label="Go back">
          <ArrowLeft className="size-4" />
        </Button>
        <h1 className="min-w-0 truncate text-sm font-semibold">{displayTask.title}</h1>
        <span className="h-4 w-px bg-[color:var(--border-subtle)]" />
        <span className="text-xs text-[var(--muted-foreground)]">{displayTask.description || 'No description'}</span>
      ...
      <Panel className="mt-3">
        ...
        <StickyActionBar>
          ...
          >
            <SelectTrigger aria-label="Move task status" size="sm" className="w-auto text-xs">
              <MoveRight className="mr-1 size-3.5" />
              <SelectValue placeholder="Move to..." />
            </SelectTrigger>
            <SelectContent>
              {getAllowedTaskTransitions(displayTask.status as TaskStatus)
                .map((status) => (
        ...
        <div className="grid gap-5 p-5 xl:grid-cols-[minmax(0,1fr)_20rem]">
          <Panel className="overflow-hidden border-none shadow-none">
            <Tabs value={activeTab} onValueChange={(value) => { if (value === 'review' && !showReviewTab) return; setActiveTab(value as WorkspaceTab); }}>
                ...
                <div className="grid gap-4">
                  <InspectorSection title="Description">
                    <p className="text-sm leading-6 text-[var(--muted-foreground)]">
                      {displayTask.description || 'No written description yet.'}
                    </p>
                  </InspectorSection>
                    ...
                    {criteria.length > 0 ? (
                      <div className="space-y-2">
                        {criteria.map((criterion, index) => {
                          const verdict = displayTask.review_verdicts?.find((v) => v.criterion_index === index);
                          return (
                            <div
                              key={criterion}
              ...
              <TabsContent value="changes" className="p-5">
                <div className="grid gap-4">
                  {displayTask.has_workspace ? (
                    <DiffViewer taskId={displayTask.id} taskStatus={displayTask.status} />
