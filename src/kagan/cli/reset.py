...
def _shutdown_server(port: int) -> bool:
    """Detect a running kagan server on *port* and ask it to shut down.

    Returns ``True`` if a server was found and successfully stopped,
    ...
    # 1. Check if a server is listening.
    try:
    ...
    # 3. Wait for the server to actually stop (up to 5 s).
    for _ in range(10):
        time.sleep(0.5)
        try:
...
def reset(project_name: str | None, force: bool) -> None:
    ...
    # Gracefully stop any running kagan server before resetting.
