...
class KaganAgent:
    ...
    async def _run_agent(self) -> None:
        ...
        try:
            async with (
                spawn_agent_process(
                    self,  # type: ignore[arg-type] — KaganAgent structurally implements acp.Client
                    parts[0],
                    *parts[1:],
                    env=env,
    ...
    async def request_permission(
        ...
        **kwargs: Any,
        ...
        decision = resolve_permission_decision(
            ...
            has_message_target=self._message_target is not None,
        )
        if decision.auto_approve:
            log.info(
                "[ACP] session/request_permission: %s, auto-approving",
                decision.reason.value,
            )
            return self._permission_response(
                self._select_option_id(options, "allow_once", "allow_always")
            )
        ...
        try:
            ...
            return self._permission_response(
                self._select_option_id(options, "reject_once", "reject_always")
            )

        log.info(f"[ACP] session/request_permission: UI responded with {answer.id}")
        return self._permission_response(answer.id)
    ...
    async def read_text_file(
        ...
        **kwargs: Any,
        ...
        read_path = self._resolve_project_path(path, operation="read_text_file")
    ...
    async def write_text_file(
        ...
        **kwargs: Any,
        ...
        write_path = self._resolve_project_path(path, operation="write_text_file")
    ...
    def _resolve_project_path(self, path: str, *, operation: str) -> Path | None:
        project_root = self.project_root.resolve()
        resolved_path = self._resolve_path(path)
        if resolved_path is None:
            return None
        if not resolved_path.is_relative_to(project_root):
            log.warning(f"[ACP] fs/{operation}: BLOCKED path traversal {resolved_path}")
            return None
    ...
    def _is_sensitive_path(self, path: Path) -> bool:
        return _is_sensitive_path_parts(path.name, path.suffix, path.parts)

    def _command_mentions_sensitive(self, command: str, args: list[str] | None) -> bool:
        token_candidates = [command] + (args or [])
        ...
        for token in shell_tokens:
            if self._token_mentions_sensitive_path(token):
                return True
            if "=" in token:
                _, value = token.split("=", 1)
                if self._token_mentions_sensitive_path(value):
                    return True
        return False

    def _token_mentions_sensitive_path(self, token: str) -> bool:
        from pathlib import Path
        ...
        path_token = normalized.replace("\\", "/")
        path = Path(path_token)
        return _is_sensitive_path_parts(path.name, path.suffix, path.parts)

    def _select_option_id(self, options: list[PermissionOption], *kinds: str) -> str | None:
        for kind in kinds:
            option_id = _find_option_id(options, kind)
    ...
    def _permission_response(self, option_id: str | None) -> RequestPermissionResponse:
        if option_id:
...
def _is_sensitive_path_parts(name: str, suffix: str, parts: tuple[str, ...] | list[str]) -> bool:
    lowered_name = name.lower()
    if lowered_name == ".env" or lowered_name.startswith(".env."):
        return True
    if lowered_name in _SENSITIVE_FILENAMES:
        return True
    if suffix.lower() in _SENSITIVE_EXTENSIONS:
        return True
    return any(part.lower() in _SENSITIVE_DIRS for part in parts)
