...
async def _run_install(
    ...
    try:
        ...
        if exc.code == "PROCESS_NONZERO_EXIT":
            detail = (exc.stderr or exc.stdout or exc.detail or "").strip()
            if not detail:
                detail = "Installation command exited with a non-zero status"
            return False, f"Installation failed: {detail}"
        return False, f"Installation failed for {agent_name}: {exc}"
