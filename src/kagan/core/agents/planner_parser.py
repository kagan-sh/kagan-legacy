...
_PLAN_TOOL_NAMES = (PLAN_SUBMIT_TOOL_NAME, PLAN_TASKS_TOOL_NAME)
...
def _is_kagan_plan_submit_call(tool_call: ToolCall | dict[str, Any]) -> bool:
    """Match plan_submit or plan_tasks (orchestrator) tool calls."""
    for raw_name in _candidate_tool_names(tool_call):
        raw_lower = raw_name.strip().lower()
        for plan_tool in _PLAN_TOOL_NAMES:
            if raw_lower.startswith(f"mcp__{_KAGAN_MCP_SERVER_NAME}__{plan_tool}"):
                return True
            if raw_lower.startswith(f"{_KAGAN_MCP_SERVER_NAME}_{plan_tool}"):
                return True
            if raw_lower.startswith(plan_tool):
                return True
            if (
                f"tool={plan_tool}" in raw_lower
                or f"name={plan_tool}" in raw_lower
                or f"toolname={plan_tool}" in raw_lower
            ):
                return True

        normalized = _normalize_tool_name(raw_name)
        if normalized in _PLAN_TOOL_NAMES:
            return True
        if normalized in {f"{_KAGAN_MCP_SERVER_NAME}_{t}" for t in _PLAN_TOOL_NAMES} | {
            f"mcp__{_KAGAN_MCP_SERVER_NAME}__{t}" for t in _PLAN_TOOL_NAMES
        }:
            return True
    return False
