...
from kagan.core.policy import expose
...
# ── Extracted mixins (re-exported for backward compatibility) ─────────
from kagan.core._plugin_ui import PluginApiMixin, PluginUiApiMixin
...
class TaskApiMixin:
    ...
    async def delete_task(self, task_id: str) -> tuple[bool, str]:
        ...
        if task is None:
            return False, f"Task {task_id} not found. Check task_id with task_list."
...
class KaganAPI(TaskApiMixin, ProjectApiMixin, AutomationApiMixin, PluginApiMixin, PluginUiApiMixin):
    ...
    async def get_instrumentation(self) -> dict[str, Any]:
        ...
        Overrides mixin implementation to preserve monkeypatch-ability on
