...
def check_criteria_coverage(
    ...
    """Return a list of acceptance criteria that appear unaddressed.

    Heuristic: a criterion is considered 'mentioned' if any of its key words
    (>=5 chars) appear in the agent_summary (case-insensitive). Returns uncovered
    criteria as a list of strings. Returns empty list when all seem covered or
    when there are no criteria.

    This is intentionally lenient — a false negative (saying all covered when
    they're not) is better than blocking valid transitions.
    """
    ...
    for criterion in acceptance_criteria:
        words = [w for w in criterion.lower().split() if len(w) >= 5]
        if not words:
            continue  # too short to meaningfully match
        matched = any(w in summary_lower for w in words)
        if not matched:
            uncovered.append(criterion)
    return uncovered


def format_criteria_warning(uncovered: list[str]) -> str:
    """Format a scratchpad warning for unaddressed acceptance criteria."""
    if not uncovered:
        return ""
    lines = ["[CRITERIA REVIEW NOTE] The following acceptance criteria may need verification:"]
    for i, criterion in enumerate(uncovered, 1):
        lines.append(f"  {i}. {criterion}")
    lines.append("Please confirm these are addressed before approving.")
    return "\n".join(lines)
