...
def skeleton_manifest(base_branch: str = "main") -> str:
    ...
    return (
        ...
        "\n"
        "# agents.<cli>: the builder/reviewer model each agent CLI runs with, keyed by the\n"
        "# CLI name (claude | codex | opencode | kimi). The value is passed verbatim to that\n"
        "# CLI — write its OWN model id (claude takes opus/sonnet/haiku; opencode takes\n"
        "# opencode/claude-*; codex/kimi take their vendor-native ids). reviewer unset\n"
        "# DISABLES the validator; a different size of the same vendor is fine. Find ids:\n"
        "# claude `--help` or /model; opencode `opencode models`; codex `--help` / OpenAI\n"
        "# docs; kimi docs.\n"
        "# agents:\n"
        "#   claude:\n"
        "#     builder: sonnet\n"
