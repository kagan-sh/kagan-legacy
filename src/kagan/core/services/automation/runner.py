...
from kagan.core.domain.task_rules import check_criteria_coverage, format_criteria_warning
...
if TYPE_CHECKING:
    ...
    from kagan.core.services.runtime import RuntimeServiceImpl, RuntimeTaskView
...
# ---------------------------------------------------------------------------
# Queued message service (extracted to _queue.py, re-exported here)
# ---------------------------------------------------------------------------

from kagan.core.services.automation._queue import (
    DEFAULT_QUEUE_LANE,
    QueuedMessage,
...
# Lower interval keeps chat output visibly "live" in CLI/TUI rather than
# arriving in coarse batches while still batching enough to avoid busy looping.
_STREAM_LOG_FLUSH_INTERVAL_SECONDS = 0.05
...
def _emit_stream_chunks(
    ...
    """Emit StreamChunk events to Wire for newly buffered agent messages.

    Returns the updated stream_index (number of messages already emitted).
    """
    if wire is None:
        return stream_index

    from kagan.core.acp.messages import AgentUpdate
    from kagan.core.wire.events import StreamChunk

    buffered = agent.get_messages()
    for msg in buffered[stream_index:]:
        if isinstance(msg, AgentUpdate) and msg.text:
            wire.emit(StreamChunk(task_id=task_id, text=msg.text))
    return len(buffered)
    ...
    *,
    ...
    try:
        while True:
            ...
            stream_index = _emit_stream_chunks(agent, wire, task_id, stream_index)
            if done:
                break
        ...
        # Flush any remaining stream chunks after agent finishes.
        _emit_stream_chunks(agent, wire, task_id, stream_index)
...
def is_auto_task(task_type: TaskType) -> bool:
    ...
    return task_type in {TaskType.AUTO, TaskType.PAIR}
...
class AutomationReviewer:
    ...
    async def run_review(
        ...
        review_persona = self._config.general.pr_reviewer_persona
        await self._record_review_persona_instance(
            task_id=task.id,
            execution_id=execution_id,
            persona=review_persona,
        )
        prompt = await self._build_review_prompt(task, persona=review_persona)
    ...
    async def _handle_complete(self, task: TaskLike) -> None:
        ...
        # Soft acceptance-criteria gate: warn in scratchpad when criteria appear unaddressed.
        if task.acceptance_criteria:
            agent_scratchpad = await self._tasks.get_scratchpad(task.id)
            uncovered = check_criteria_coverage(
                task.acceptance_criteria, agent_summary=agent_scratchpad
            )
            if uncovered:
                warning = format_criteria_warning(uncovered)
                updated_scratchpad = (
                    f"{agent_scratchpad}\n\n{warning}" if agent_scratchpad else warning
                )
                await self._tasks.update_scratchpad(task.id, updated_scratchpad)
                log.info(f"Task {task.id}: {len(uncovered)} acceptance criteria may be unaddressed")
    ...
    async def _build_review_prompt(self, task: TaskLike, *, persona: str | None = None) -> str:
    ...
    async def _record_review_persona_instance(
        self,
        *,
        task_id: str,
        execution_id: str,
        ...
        """Persist review persona usage metadata with explicit task linkage."""
        ...
        existing_raw = metadata.get("review_persona_instances")
        ...
        instances.append(
            {
                "task_id": task_id,
                "iteration": len(instances) + 1,
                "persona": persona,
                "recorded_at": utc_now().isoformat(),
            }
        )
        metadata["review_persona_instances"] = instances
...
class AutomationEngine:
    ...
    def __init__(
        ...
        *,
        ...
        queued_message_service: QueuedMessageServiceImpl | None = None,
    ...
    async def _admit_pending_spawns(self) -> None:
        ...
        async with self._pending_spawn_lock:
            ...
            if max_agents < 1:
                log.warning(
                    "Invalid max_concurrent_agents=%s; using fallback capacity=1",
                    max_agents,
                )
                max_agents = 1
    ...
    async def _engine_send_prompt_with_incremental_persistence(
        ...
        *,
        ...
        if self._executions is None:
            await agent.send_prompt(prompt)
            # Still emit any buffered output to Wire even without persistence.
            _emit_stream_chunks(agent, self._wire, task_id, 0)
            return False
    ...
    async def _run_execution(
        ...
        await self._record_worker_persona_instance(
            task_id=task.id,
            execution_id=execution_id,
            run_count=run_count,
        )
        ...
        prompt = build_prompt(
            ...
            persona=self._config.general.worker_persona,
            user_name=user_name,
    ...
    async def _append_queued_message_to_scratchpad(self, task_id: str, content: str) -> None:
        ...
        safe_content = redact_sensitive_text(content, redact_pii=True)
        user_note = f"\n\n--- USER MESSAGE ---\n{truncate_queue_payload(safe_content)}"
    ...
    async def _record_worker_persona_instance(
        self,
        *,
        task_id: str,
        execution_id: str,
        ...
        """Persist worker persona usage metadata keyed by task and run iteration."""
        if self._executions is None:
            return
        execution = await self._executions.get_execution(execution_id)
        if execution is None:
            return
        metadata = dict(execution.metadata_ or {})
        metadata["worker_persona_instance"] = {
            "task_id": task_id,
            "run_count": run_count,
            "persona": self._config.general.worker_persona,
            "recorded_at": utc_now().isoformat(),
...
class AutomationServiceImpl:
    ...
    def __init__(
        ...
        from kagan.core.agents.agent_factory import create_agent as _create_agent
        ...
        self._engine = AutomationEngine(
            ...
            on_error=on_error,
            notifier=notifier,
            agent_factory=agent_factory or _create_agent,
            event_bus=event_bus,
