...
class ReviewActionsMixin:
    ...
    def _actions_job_result_message(
        ...
        *,
        ...
        if payload is not None and not bool(payload.get("success", False)):
            return (
                job_message(terminal, failure_msg),
                "warning",
            )
        ...
        return (
            job_message(terminal, success_msg),
            "information",
        )
    ...
    def action_show_ai_review(self) -> None:
        """Switch to the review ACP session."""
        selector = getattr(self, "_session_select", None)
    ...
    def action_show_agent_output(self) -> None:
        """Switch to the implementation ACP session."""
        selector = getattr(self, "_session_select", None)
        if callable(selector):
    ...
    def action_cycle_session(self) -> None:
        """Cycle active ACP session."""
        cycler = getattr(self, "_session_cycle", None)
        if callable(cycler):
            cycler()
    ...
    async def action_generate_review(self) -> None:
        ...
        session_select = getattr(self, "_session_select", None)
        if callable(session_select):
            session_select("review")
    ...
    async def action_start_agent_output(self) -> None:
        """Start AUTO execution from the implementation session."""
    ...
    async def action_stop_agent_output(self) -> None:
        """Stop AUTO execution from the implementation session."""
        latest = await self.ctx.api.get_task(self._task_model.id)  # type: ignore[attr-defined]
        if latest is None:
            self.notify("Task no longer exists", severity="error")  # type: ignore[attr-defined]
            return
