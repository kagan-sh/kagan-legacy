...
if TYPE_CHECKING:
    ...
    class SessionTaskLike(TaskLike, Protocol):
        project_id: str
...
class KanbanSessionController:
    ...
    async def provision_workspace_for_active_repo(
        self, task: SessionTaskLike
        ...
        try:
            repo_inputs = repo_details_to_workspace_inputs([repo])
    ...
    async def handle_missing_agent(self, task: SessionTaskLike, agent_config: AgentConfig) -> str:
    ...
    async def update_task_agent(self, task: SessionTaskLike, agent_short_name: str) -> None:
    ...
    async def confirm_start_auto_task(self, task: SessionTaskLike) -> bool:
    ...
    async def confirm_attach_pair_session(self, task: SessionTaskLike) -> bool:
        from kagan.core.constants import NOTIFICATION_TITLE_MAX_LENGTH
    ...
    async def open_session_flow(self, task: SessionTaskLike) -> None:
    ...
    async def _open_read_only_output_for_terminal_status(self, task: SessionTaskLike) -> None:
    ...
    async def _open_auto_session_flow(self, task: SessionTaskLike) -> None:
    ...
    async def _open_blocked_auto_output_if_needed(self, task: SessionTaskLike) -> bool:
        """Open AUTO output modal and surface blocked-state context."""
        ...
        self.screen.notify(
            blocked_reason or "Task is blocked by overlapping AUTO changes.",
            severity="warning",
        )
        return True

    async def _start_and_open_auto_output(self, task: SessionTaskLike) -> None:
    ...
    async def _resume_or_start_in_progress_auto(
        ...
        *,
        ...
        if start_requested:
            ...
            return
        self.screen.notify(
            "No active AUTO run detected. Press 'a' to start, "
            "'s' to stop, or queue follow-up text.",
            severity="information",
        )

    async def _open_pair_session_flow(self, task: SessionTaskLike) -> None:
    ...
    async def _ensure_pair_agent_available(self, task: SessionTaskLike) -> SessionTaskLike | None:
    ...
    async def _ensure_pair_session_exists(
        self, task: SessionTaskLike, workspace_path: Path
    ) -> None:
    ...
    def resolve_pair_terminal_backend(self, task: SessionTaskLike) -> str:
    ...
    async def ensure_pair_terminal_backend_ready(
        self, task: SessionTaskLike
    ) -> PairBackendReadiness:
        ...
        if backend in {"vscode", "cursor", "nvim", "windsurf", "kiro", "antigravity"}:
            if check_terminal_installed(backend):
                return PairBackendReadiness(success=True, backend=backend)
            self.screen.notify(
                f"{backend} is not installed. {get_manual_install_fallback(backend)}",
                severity="warning",
            )
        ...
        if backend == "tmux":
            ...
            if is_windows:
                ...
                self.screen.notify(
                    "PAIR cancelled: install Neovim, VS Code, Cursor, Windsurf, Kiro, "
                    "or Antigravity to continue on Windows.",
                    severity="warning",
                )
                return PairBackendReadiness(
                    success=False,
            ...
            if fallback is not None:
                await self.screen.ctx.api.update_task(task.id, terminal_backend=fallback)
                fallback_hint = ""
                if fallback in {"vscode", "cursor", "windsurf", "kiro", "antigravity"}:
                    fallback_hint = (
                        " VS Code: https://code.visualstudio.com/download "
                        "Cursor: https://cursor.com/downloads"
                    )
                elif fallback == "nvim":
                    fallback_hint = " Neovim: https://neovim.io"
                self.screen.notify(
                    f"tmux not installed. Using fallback launcher {fallback}.{fallback_hint}",
                    severity="information",
                )
                return PairBackendReadiness(success=True, backend=fallback)
            self.screen.notify(
                "PAIR cancelled: install tmux (recommended), Neovim, or install "
                "VS Code/Cursor/Windsurf/Kiro/Antigravity for external development.",
                severity="warning",
            )
            return PairBackendReadiness(
                success=False,
        ...
        try:
            ...
            with self.screen.app.suspend():
                if backend == "tmux":
                    ...
                    attached = await self._attach_nvim_session_local(workspace_path)
                else:
            ...
            if backend in {"vscode", "cursor"}:
                ...
                if backend == "vscode":
                    self.screen.notify(
                        "Workspace opened in VS Code. Auto-chat is attempted when Copilot Chat "
                        "is installed.",
                        severity="information",
                    )
                    await self._await_vscode_pair_acknowledgement(prompt_path)
                else:
                    self.screen.notify(
    ...
    async def _await_vscode_pair_acknowledgement(self, prompt_path: Path) -> None:
        confirmed = await self.ask_confirmation(
            "VS Code PAIR Handshake",
            "Kagan attempted to auto-start Copilot Chat in agent mode. "
            f"If chat did not open, run it manually using startup prompt: {prompt_path}. "
            "After the first assistant reply, return and press Enter to acknowledge.",
        )
        if confirmed:
            self.screen.notify("PAIR handshake acknowledged.", severity="information")
        else:
            self.screen.notify(
                "PAIR handshake not acknowledged. Re-open with Enter when ready.",
                severity="information",
            )
    ...
    async def _attach_nvim_session_local(self, workspace_path: Path | None) -> bool:
        """Attach to Neovim from the TUI process so terminal IO stays interactive."""
        if workspace_path is None:
            return False
        prompt_path = self.startup_prompt_path_hint(workspace_path)
        ...
        try:
            proc = await spawn_exec("nvim", target, "-c", chat_bootstrap, cwd=str(workspace_path))
        except OSError:
            return False
        returncode = await proc.wait()
        return returncode == 0
