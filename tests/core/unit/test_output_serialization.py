...
def test_serialize_agent_messages_keeps_streamed_text_chunks() -> None:
    payload = serialize_agent_messages(
        [
            messages.AgentUpdate("text", "Hello"),
            messages.AgentUpdate("text", " world"),
            messages.AgentUpdate("terminal", "$ ls"),
        ]
    )
    ...
    assert json.loads(payload) == {
        "messages": [
            ...
            {"type": "response", "content": "$ ls"},
        ]
    }


def test_serialize_agent_messages_returns_none_for_non_renderable_slice() -> None:
    payload = serialize_agent_messages([messages.AgentComplete()])
