...
async def test_install_terminal_returns_manual_fallback_when_auto_installer_unavailable() -> None:
    with (
        patch("kagan.terminals.installer.check_terminal_installed", return_value=False),
        patch("kagan.terminals.installer._get_tmux_install_command", return_value=None),
    ):
