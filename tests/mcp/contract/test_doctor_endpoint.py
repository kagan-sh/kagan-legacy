...
@pytest.mark.asyncio
async def test_doctor_ok_when_all_checks_pass() -> None:
    ...
    with patch("kagan.server._system_routes.run_doctor_checks", return_value=checks):
...
@pytest.mark.asyncio
async def test_doctor_not_ok_when_any_check_fails() -> None:
...
@pytest.mark.asyncio
async def test_doctor_response_shape_matches_doctor_report_response() -> None:
...
@pytest.mark.asyncio
async def test_doctor_is_blocking_derived_from_status() -> None:
...
@pytest.mark.asyncio
async def test_doctor_category_defaults_to_core_without_field() -> None:
    ...
    with patch("kagan.server._system_routes.run_doctor_checks", return_value=stub_checks):
        response = await endpoint(make_request("GET", "/api/doctor"))
