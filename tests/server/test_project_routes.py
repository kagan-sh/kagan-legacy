...
def test_registers_project_collection_routes() -> None:
    mcp = _make_api_server()

    assert _has_route(mcp, "/api/projects", "GET")
    assert _has_route(mcp, "/api/projects", "POST")


def test_registers_project_activate_and_delete_routes() -> None:
    mcp = _make_api_server()

    assert _has_route(mcp, "/api/projects/{project_id}/activate", "POST")
    assert _has_route(mcp, "/api/projects/{project_id}", "DELETE")
