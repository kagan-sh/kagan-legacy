...
def test_registers_settings_route() -> None:
    mcp = _make_api_server()

    assert _has_route(mcp, "/api/settings", "GET")


def test_registers_resolved_settings_route() -> None:
    mcp = _make_api_server()

    assert _has_route(mcp, "/api/settings/resolved", "GET")


def test_registers_preflight_route() -> None:
    mcp = _make_api_server()

    assert _has_route(mcp, "/api/preflight", "GET")
