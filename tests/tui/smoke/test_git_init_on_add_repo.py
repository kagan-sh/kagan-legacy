...
from kagan.core.git_utils import GitError, GitInitResult
...
def _make_new_project_app(mock_project_service: Any):
    ...
    class NewProjectTestApp(App):
        ...
        @property
        def ctx(self):
            api = SimpleNamespace(create_project=mock_project_service.create_project)
            return SimpleNamespace(project_service=mock_project_service, api=api)
...
def _make_repo_picker_app(mock_project_service: Any):
    ...
    api = SimpleNamespace(
        add_repo=mock_project_service.add_repo_to_project,
        get_project_repos=mock_project_service.get_project_repos,
...
class TestNewProjectModalGitInit:
    ...
    async def test_empty_folder_auto_init_creates_project(self, tmp_path):
        ...
        async with app.run_test() as pilot:
            ...
            with patch(
                "kagan.core.git_utils.init_git_repo",
    ...
    async def test_git_repo_path_skips_init(self, tmp_path):
        ...
        async with app.run_test() as pilot:
            ...
            with patch("kagan.core.git_utils.init_git_repo", new_callable=AsyncMock) as mock_init:
                await pilot.click("#btn-create")
                await pilot.pause()
