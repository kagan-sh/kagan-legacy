...
class TestWelcomeScreen:
    def test_welcome_journey(
        ...
        monkeypatch.setattr("kagan.tui.ui.screens.welcome.utc_now", lambda: _FIXED_NOW)
        ...
        async def run_flow() -> dict[str, str]:
            ...
            async with app.run_test(headless=True, size=(cols, rows)) as pilot:
                ...
                await pilot.app.push_screen(
                    WelcomeScreen(
                        suggest_cwd=True,
                        cwd_path="/Users/dev/my-folder",
                        cwd_is_git_repo=False,
                    )
                )
                await pilot.pause()
                snapshots.update(await execute_test_actions(pilot, ["shot(cwd_banner_no_git)"]))
                await pilot.app.pop_screen()
                await pilot.app.push_screen(
                    WelcomeScreen(
                        suggest_cwd=False,
